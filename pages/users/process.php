...
if (isset($_POST['user_submit'])) {
	...
	if ($valid) {
		...
		$conn->query("INSERT INTO `users`(`name`, `mobile`, `username`, `password`, `national_id`, `kra_pin`, `email`, `user_group`,`status`,`vendor`,`userId`) VALUES ('$name','$phone','$username','$password','$id_no','$kra','$email','$group','$status','CGS','$userId') ") or die($conn->error);
...
if (isset($_GET['user'])) {
	...
	if (mysqli_num_rows($user) == 1) {
		...
		$name = $row_user_update['name'];
		$phone = $row_user_update['mobile'];
		$id_no = $row_user_update['national_id'];
		$kra = $row_user_update['kra_pin'];
...
if (isset($_POST['user_update_submit'])) {
	...
	if(empty($name)){
		$name_err = "Please enter a name.";
		$valid = false;
		$_SESSION["user_update_errors"]["name_err"] = $name_err;
		header('location: user_update.php?user='.$user_id);
	}

	if(!is_numeric($phone)){
		$mobile_err = "Please valid mobile number.";
		$valid = false;
		$_SESSION["user_update_errors"]["mobile_err"] = $mobile_err;
		header('location: user_update.php?user='.$user_id);
	}
	if(strlen($phone) < 10){
		$mobile_err = "Phone number must have atleast 10 characters.";
	...
	if(!is_numeric($id_no)){
		$id_err = "Please valid ID number.";
		$valid = false;
		$_SESSION["user_update_errors"]["id_err"] = $id_err;
		header('location: user_update.php?user='.$user_id);
	}
	if(empty($username)){
		$username_err = "Please enter a unique username.";
		$valid = false;
		$_SESSION["user_update_errors"]["username_err"] = $username_err;
		header('location: user_update.php?user='.$user_id);
	}
	$res = $conn->query("SELECT * FROM users WHERE username='$username' AND id <> '$user_id' ") or die($conn->error);
	if(mysqli_num_rows($res) >= 1){
		$username_err = "Username is already taken.";
		$valid = false;
		$_SESSION["user_update_errors"]["username_err"] = $username_err;
	...
	if(!empty($password) or !empty($cpassword)) {
		...
		if(empty($password)){
			$pass_err = "Please enter a strong password.";
			$valid = false;
			$_SESSION["user_update_errors"]["pass_err"] = $pass_err;
			header('location: user_update.php?user='.$user_id);
		}
		if(strlen($password) < 6){
		...
		if(empty($cpassword)){
			...
			$cpass_err = "Please confirm your password.";
			$valid = false;
			$_SESSION["user_update_errors"]["cpass_err"] = $cpass_err;
			header('location: user_update.php?user='.$user_id);
		...
		if($valid) {
			...
			unset($_SESSION["user_update_errors"]);
