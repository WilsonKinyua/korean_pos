                        ...
                        if($_POST['id'] == 'all' and $_POST["store_code"] == "all") {
                            ...
                            $res = $conn->query(
                                ...
                                "SELECT *, 
                                    ...
                                    IFNULL((stkmobile.issued_quantity - nsale.sold), 0) returned,
                                    IFNULL(stkmobile.issued_quantity, 0) issued_quantity,
                                    IFNULL(stkmobile.unit_cost, 0) unit_cost
                                    FROM
                                    ...
                                    (
                                        SELECT new_sale.salesman, new_sale.`product`, IFNULL(SUM(new_sale.quantity), 0) AS sold
                                      FROM new_sale
                            ...
                            $res = $conn->query(
                                ...
                                INNER JOIN products ON products.id=stock_mobile.product
                                INNER JOIN users ON users.id=stock_mobile.store_owner
                                WHERE stock_mobile.store_owner='$id' AND products.vendor='$store_code' AND stock_mobile.date BETWEEN CAST('$from' AS DATE) AND DATE_ADD(CAST('$to' AS DATE), INTERVAL 1 DAY)"
