    ...
    $sql="SELECT * FROM products WHERE id IN (";
          ...
          while($row=$query->fetch_assoc()){
                ...
                if (mysqli_num_rows($res) > 0) {
                    ...
                    //   echo "<script type='text/javascript'>alert('$item Requested Quantity $quantity Above available stock $avstock'); </script>";
                      ...
                      if (mysqli_num_rows($res1) > 0) {
                        ...
                        $conn->query("INSERT INTO `stock_mobile_additions`(`store_owner`, `product`, `quantity_added`, `doc_number`, `assigned_by`) VALUES ('$salesman','$id','$quantity0','$doc_number', $userId)") or die($conn->error);
                        
                      } else{ 

                        $conn->query("INSERT INTO `stock_mobile`(`store_owner`, `product`, `quantity_available`) VALUES ('$salesman','$id','$quantity')") or die($conn->error);
                        $conn->query("INSERT INTO `stock_mobile_additions`(`store_owner`, `product`, `quantity_added`, `doc_number`) VALUES ('$salesman','$id','$quantity', '$doc_number')") or die($conn->error);
             ...
             //   echo "<script>alert('Product is out of stock')</script>";
              $_COOKIE['purchase_error'] .= $nprod .' is out of stock Kindly Purchase';
              }
            }
...
if(isset($_POST['save_request_print'])){
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <?php if(isset($_COOKIE['purchase_success']) and $_COOKIE['purchase_success']) {?>
              ...
              <div id="<?= isset($_SESSION['dont_close_alert']) ? $_SESSION['dont_close_alert'] : 'notif-' ?>"
                   class="alert alert-success text-center alert-dismissible fade show mt-4" role="alert">
          ...
          <?php if(isset($_COOKIE['quantity_available_exceeded']) and $_COOKIE['quantity_available_exceeded']) {?>
              ...
              <div id="<?= isset($_SESSION['dont_close_alert']) ? $_SESSION['dont_close_alert'] : 'notif' ?>" class="alert alert-danger text-center alert-dismissible fade show mt-4" role="alert">
          ...
          <?php } ?>
              <?php if(isset($_COOKIE['purchase_error']) and $_COOKIE['purchase_error']) {?>

                  <div id="<?= isset($_SESSION['dont_close_alert']) ? $_SESSION['dont_close_alert'] : 'notif-' ?>" class="alert alert-danger text-center alert-dismissible fade show mt-4" role="alert">
                      <strong><?php echo $_COOKIE['purchase_error'] ?></strong>
                      <button type="button" class="close text-white" data-dismiss="alert" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                      </button>
                  </div>
          ...
          <div class="card card-primary">
                ...
                <div class="card-body">
                  <div class="form-group">
                    <div class="row">
                      <div class="col-md-4">
                          <?php
                          $query = $conn->query('SELECT id FROM stock_mobile_additions ORDER BY id DESC LIMIT 1');
                          $last = $query->fetch_assoc();
                          $lastId = isset($last['id']) ? (int)$last['id'] : 0;
                          $docId = str_pad($lastId + 1, 6, '0', STR_PAD_LEFT);
                          ?>
                        <label>Receipt Number: </label>
                        <input type="text" name="doc_number" value="ASN-<?php echo $docId; ?>" class="form-control" readonly>
                      </div>
                      <div class="col-md-4">
              ...
              if (isset($_SESSION['dont_close_alert'])){ ?>
                  <button hidden id="print" onclick="window.open('request_print.php?request_id=<?= $_SESSION['dont_close_alert'] ?>', '_blank');"></button>
              <?php }
              ?>
          </div>
        </div>
