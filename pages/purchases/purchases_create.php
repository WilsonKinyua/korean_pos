...
if (isset($_GET['tax'])) {
    $_SESSION['purchase_tax'] = $_GET['tax'];
}
...
if(isset($_POST['clear_cart'])){
    unset($_SESSION['supplier'], $_SESSION['purchases_cart'], $_SESSION['purchase_tax']);
    header('location: purchases_create.php');
}
$date = date('Y-m-d');
$supplier_err = $tax_err = "";
$valid = true;

if(isset($_POST['save_purchase'])){
    $doc_number = 'DOC-PRCHS'.round(microtime(true)*1000);
    $supplier = isset($_POST['supplier']) ? $_POST['supplier']: '';
    $taxation = isset($_POST['tax'])? $_POST['tax']: '';
    ...
    if (empty($taxation)) {
        $tax_err = "Taxation method must be selected";
        $valid = false;
    }
    // $s = $_POST['salesman'];
...
// var_dump($_POST);
    if ($valid) {
        ...
        $sql="SELECT products.*,tax_rates.tax_rate  FROM products LEFT JOIN tax_rates on products.tax_id = tax_rates.id WHERE products.id IN (";
        ...
        while($row=$query->fetch_assoc()){
            ...
            $tax = 0;
            if (isset($_SESSION['purchase_tax']) && $_SESSION['purchase_tax'] == 1){
                $tax = $subtotal - ($subtotal / (1 + $row['tax_rate'] / 100));
            }elseif (isset($_SESSION['purchase_tax']) && $_SESSION['purchase_tax'] == 2){
                $tax = $subtotal * $row['tax_rate'] / 100;
                $subtotal += $tax;
            }
            ...
            $conn->query("INSERT INTO `stock_detail`(`store`,`entity`, `doc_number`, `product`, `trn_qty`, `total_amount`,`trn_type`,`bal_qty`,`date`,`audituser`, `line_tax_total`, `received_by`) 
                        VALUES ('$supplier','$supplier','$doc_number','$id','$quantity','$subtotal','1','0','$date','$username', '$tax', '$userId')") or die($conn->error);
...
if(isset($_POST['save_purchase_print'])){
    ...
    if ($valid) {
        ...
        header('location: purchase_print.php');
        exit();
    }
    ...
    <div class="content-wrapper">
        <script type="text/javascript">
            ...
            function changeTax(){
                ...
                window.location.href = "purchases_create.php?tax="+tax;
            }
        </script>
        <!-- Main content -->
                ...
                <div class="row">
                    <div class="col-md-12">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        <div class="row">
                                            ...
                                            <div class="col-md-3">
                                                <label for="customer_name">Tax: </label>
                                                <select class="form-control" name="tax" id="tax" onchange="changeTax()">
                                                    <option value="" disabled selected>--SELECT TAXATION--</option>
                                                    <option value="1" <?= isset($_SESSION['purchase_tax']) && $_SESSION['purchase_tax'] == 1 ? 'selected': '' ?>>Inclusive</option>
                                                    <option value="2" <?= isset($_SESSION['purchase_tax']) && $_SESSION['purchase_tax'] == 2 ? 'selected': '' ?>>Exclusive</option>
                                                </select>
                                                <span style="color:red;"><?php echo $tax_err; ?></span>
                                            </div>
                                            <div class="col-md-3">
                                            ...
                                            if (!isset($_SESSION['purchases_cart']) || empty($_SESSION['purchases_cart'])) {
                                                ...
                                                while($row=$query->fetch_assoc()){
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>Kshs. <?php echo number_format($tax, 2) ?></td>
                                                        <td>Kshs. <?php echo $subtotal ?></td>
                                                        <td><a href="purchases_create.php?page=products&action=remove&id=<?php echo $row['id'] ?>"><i class="fas fa-trash-alt"></i></a></td>
                                                    </tr>
                                                <?php }?>
                                                <tr><td colspan="4"></td>
                                                    <th>Total Tax: <?php echo number_format($totaltax, 2) ?></th>
                                                    <td>&nbsp;</td>
                                                </tr>
                                                <tr><td colspan="4"></td>
                        ...
                        if (isset($_SESSION['dont_close_alert'])){ ?>
                            <button hidden id="print" onclick="window.open('purchase_print.php?purchase_id=<?= $_SESSION['dont_close_alert'] ?>', '_blank');"></button>
                        <?php }
                        ?>
                    </div>
                </div>
