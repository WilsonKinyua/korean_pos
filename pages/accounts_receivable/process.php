        ...
        while($row=$query->fetch_assoc()){
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if ($row['quantity_available']<$quantity) {
                    ...
                    $conn->query("INSERT INTO `invoice_lines`( `product`, `quantity`, `line_amount`, `invoice_id`, `line_coa_id`, `line_tax_total`) 
                                VALUES ('$id','$quantity','$neg','$invoice_id','510', '$tax')") or die($conn->error);
                    $conn->commit();
